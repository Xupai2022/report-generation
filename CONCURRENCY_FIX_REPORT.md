...
**新增端点**: `POST /cleanup?max_age_hours=24`
...
**测试脚本**: [test_concurrency.py](test_concurrency.py)
...
# 生成预览
response = requests.get(f"http://localhost:8000/preview?job_id={job_id}")
preview_urls = response.json()["images"]
...
# 手动清理旧会话 (可选)
requests.post("http://localhost:8000/cleanup?max_age_hours=24")
```
...
- [test_concurrency.py](test_concurrency.py) - 并发测试脚本
